...
function Shop({ userId, setUserId }) {
    ...
    useEffect(() => {
        const fetchShopCategories = async () => {
            try {
                const response = await axios.get('http://localhost:8081/userShop/Getshop/categories');
                setShopCategories(response.data);


                response.data.forEach(async (category) => {
                    try {
                        const productsResponse = await axios.get(`http://localhost:8081/userShop/GetProduct/${category.id}`);
                                    ...
                                    <div key={product.id} className="col-md-3 col-sm-6">
                                        <div className="product-grid" >
                                            <div className="product-image">
                                                <Link to={`/SingleProduct/${product.id}`} className="image">
                                                    <div className="card-image-grid" style={{ width: "100%", height: "250px", overflow: "hidden" }}>
                                                        <img className="pic-1" src={getImageUrl(product.product_image)} alt={product.product_name} />
                                                        <img className="pic-2" src={getImageUrl(product.product_image)} alt={product.product_name} />
                                                    </div>
                                                </Link>
                                                {product.product_offerPrice && (
                                                <div className="discount-badge">
                                                    {calculateDiscountPercentage(product.product_price, product.product_offerPrice)}% OFF
                                                </div>
                                            )}
                                                <ul className="product-links">
                                                    <li><button><IoHeartOutline /></button></li>
                                                    <form onSubmit={(e) => handleFormSubmit(e, product.id, product.product_name, product.product_price, product.product_brand, product.product_image, product.product_offerPrice)}>
                                                        <li><button type="submit"><IoCart /></button></li>
                                                    </form>
                                                </ul>
                                            </div>
                                            <div className="product-content">
                                                <Link to={`/SingleProduct/${product.id}`} className="image" style={{ textDecoration: "none", color: "black" }}>
                                                    <h3 className="title">{product.product_name}</h3>
                                                    {product.product_offerPrice ? (
                                                        <div className="price">&#8377;{product.product_offerPrice}  <strike style={{
                                                            color: "black",
                                                            fontSize: "smaller"
                                                        }}> &#8377;{product.product_price}</strike></div>
                                                    ) : (
                                                        <>
